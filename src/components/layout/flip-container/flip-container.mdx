...
<Tip type="warning">
  If the flip container is being controlled by an external button, the button
  should be given an `aria-controls` attribute with the value of the flip
  container's `id` attribute. The button should also be given an `aria-expanded`
  attribute with the value of `true` when the flip container is visible and
