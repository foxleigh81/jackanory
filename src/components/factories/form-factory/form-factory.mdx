...
<Tip type="warning">
  You must specify at least one fieldset (legend is optional)
</Tip>
...
<Tip>
  The data you can pass to `fieldsets`, `fields` and `actions` are all based on
  the props for the `fieldset`, `InputFactory` and `Button` components
  respectively, so any of those props are valid here!.
</Tip>
