...
<Tip>
  {' '}
  Some components can have issues when rendered via the InputFactory component.
  If you are having an issue try passing the `parent` prop to the label. This
  will prevent the `htmlFor` attribute from being set which usually fixes the
  issue.{' '}
</Tip>
